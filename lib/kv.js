...
const KEYS = {
  ...
  badges: 'db:badges',
  successPlans: 'db:successPlans'
};

export async function getState() {
  const redis = getRedis();
  const [clubs, meetings, pathways, mentors, directors, strength, badges, successPlans] = await Promise.all([
    ...
    redis.get(KEYS.badges),
    redis.get(KEYS.successPlans)
  ]);
  return {
    clubs: clubs || [],
    meetings: meetings || [],
...
export async function saveBadges(v) { await getRedis().set(KEYS.badges, v); }
export async function saveSuccessPlans(v) { await getRedis().set(KEYS.successPlans, v); }
